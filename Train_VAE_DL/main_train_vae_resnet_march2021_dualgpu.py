...
path_results = os.path.expanduser('/data/volume_2/look_space/results/Feature_extraction/model_results/'+str(satellite)+'_trainedresmodel/') + folder_results + "/"

path_datasets = os.path.expanduser('/data/volume_2/look_space/data/nomask/4bands/')
...
if (total_val%2) != 0:
    train_data, validate_data, test_data = torch.utils.data.random_split(image_all, [int(0.8*(len(image_all))), int(np.ceil(0.1*(len(image_all)))), int(np.ceil(0.1*(len(image_all))))])
else:
    train_data, validate_data, test_data = torch.utils.data.random_split(image_all, [int(round(0.8*(len(image_all)))), int(round(0.1*(len(image_all)))), int(round(0.1*(len(image_all))))])
...
with open(path_results+"stats.txt", 'a') as f:
    ...
    if weighted:
        ...
        f.write("Loss not weighted,kl-0.001" + "\n")
...
writer = SummaryWriter('/data/volume_2/look_space/results/runs/resnet_experiment_1'+run_name+'/')

# Create model
resnet_vae=AutoEncoder(num_layers, pretrained, bands,patch_size).to(device)
...
def loss_function(recon_x, x, mu, logvar):
   ...
   # ssim_val =ssim(x.to(device='cuda:0'),recon_x.to(device='cuda:0'),data_range=1,size_average=True).to(device='cuda:0')
    return  MSE + (0.001*KLD)#+ssim_val #+ proj_loss.mean() #0.001-original
...
for epoch in range(epoch_nb):
    ...
    for batch_idx, img_data in enumerate(loader,0):
        ...
        optimizer.zero_grad()
        decoded, z, mu, logvar,x_l1,x_l2,x_l3,d_l3,d_l4 = resnet_vae(img_data)  
