...
import pandas as pd
from models.AE_fully_convolutional_model_resnet_march2021 import *    #we chose fully_conv or conv model
...
PATH="D:/Utrecht_macrozoobenthos_data_feb2021/Main_codes/results/model_results/ResNet/"
# We define the input and output paths
test_results = os.path.expanduser('D:/Utrecht_macrozoobenthos_data_feb2021/Data_planetscope/test_results/june30_2019/'+'modelufv.2021-12-06/') + "/"
create_dir(test_results)
path_datasets = os.path.expanduser('D:/Utrecht_macrozoobenthos_data_feb2021/Data_planetscope/maskedflats/sentinel/')
...
with open(test_results+'parameters.txt', 'w') as f:
    f.write("ae-model_ep_400_loss_20959.3850929.2021-12-06_1412.pth")
...
newRaster.SetGeoTransform([newTLCoord[0], 10, 0, newTLCoord[1], 0, -10])
...
#subset of images			
#list_image_extended=list_image_extended[3200:3400,700:1000,:]
...
resnet_vae.load_state_dict({k.replace('module.',''):v for k,v in torch.load('{}/ae-model_ep_400_loss_20959.3850929.2021-12-06_1412.pth'.format(PATH),map_location='cpu').items()})#we have added new lines now or use strict=False newly added for research cloud  because of nn.parallel
print("model loaded")
...
pad_row=(64-(H%64))//2
pad_col=(64-(W%64))//2
image=np.pad(list_image_extended,((pad_row,pad_row),(pad_col,pad_col+1),(0,0)),'edge')
print("Image shape",image.shape)
num_rows=image.shape[0]
num_cols=image.shape[1]
no_patches=((num_rows//40)-1)*((num_cols//40)-1)
...
image_final=image_final[pad_row:H+pad_row,pad_col:W+pad_col,:]
...
enco64_final=enco64_final[pad_row:H+pad_row,pad_col:W+pad_col,:]
...
recons_final=recons_final[pad_row:H+pad_row,pad_col:W+pad_col,:]
...
driver = gdal.GetDriverByName('GTiff')
newRaster2 = driver.Create(test_results+'reconstructed_image'+'_subset1'+'.tif',list_image_extended.shape[1],
list_image_extended.shape[0], input_channels, gdal.GDT_Float32)#columns,rows
newRaster1 = driver.Create(test_results+'original_image'+'_subset1'+'.tif',list_image_extended.shape[1],
...
newRaster2.SetGeoTransform([newTLCoord[0], 10, 0, newTLCoord[1], 0, -10])
newRaster1.SetGeoTransform([newTLCoord[0], 10, 0, newTLCoord[1], 0, -10])
...
driver1 = gdal.GetDriverByName('GTiff')
features=64
newfeature = driver1.Create(test_results+'Encoder64_image'+'_subset1'+'.tif',list_image_extended.shape[1],
list_image_extended.shape[0], features, gdal.GDT_Float32)
newfeature.SetProjection(prj1)
newTLCoord=gdal.ApplyGeoTransform(gt, py, px)
newfeature.SetGeoTransform([newTLCoord[0], 10, 0, newTLCoord[1], 0, -10])
